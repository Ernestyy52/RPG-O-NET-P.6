...
- **Phase status:** Inc 1 (plan) + Inc 2 (DungeonScene) done; **Inc 3 in progress** — pure boss 3-phase state machine done (+tests); **three live flag flips landed: `NEW_ZONE_RUNTIME_ENABLED` (flip #1), `COMBAT_DOMAIN_ENABLED` (flip #2), `REALTIME_COMBAT_ENABLED` (flip #3)**. Real-time combat is now the live path on World-1 floors 1–10 (`RealtimeBattle` HUD + boss 3-phase telegraph in BossScene); floors 11+ stay turn-based. Remaining Inc 3: **flip #4 `KNOWLEDGE_BREAK_ENABLED`** (wire Knowledge Break into the realtime loop — currently NOT wired) → `CLASS_KITS_ENABLED` → `SIGILS_ENABLED` → `ADAPTIVE_EXPEDITIONS_ENABLED`. Inc 4–5 pending. Build exit 0, 199 tests. **Human-at-browser verification outstanding for flips #2 (turn-based) and #3 (real-time field + boss playthrough, desktop/mobile/reduced-motion).** See `docs/execution/PHASE_14_PLAN.md` + `DECISION_LOG.md` D-013/D-014.
...
- **Feature flags:** `NEW_ZONE_RUNTIME_ENABLED` (P08) = **`true`** (flip #1 — dungeon doors on floors 5/10 live); `COMBAT_DOMAIN_ENABLED` (P07) = **`true`** (flip #2 — engine-driven BattleModal turns, all floors); `REALTIME_COMBAT_ENABLED` (P09) = **`true`** (flip #3 — World-1 floors 1–10 now fight on the real-time `RealtimeBattle` HUD + boss telegraph; floors 11+ stay turn-based via `isWorld1Floor` gate). Still `false`: `KNOWLEDGE_BREAK_ENABLED` (P10), `ADAPTIVE_EXPEDITIONS_ENABLED` (P11), `CLASS_KITS_ENABLED` (P12), `SIGILS_ENABLED` (P13), `SAVE_ENVELOPE_ENABLED`. Floors 11+ turn-based BattleModal + TowerScene renderer + legacy daily quests + legacy player store remain the live paths there. **Real-time is live but interactive in-browser playthrough is the outstanding human verification (Phaser can't render headless here).**
...
**Continue Phase 14 Inc 3 → flip #4.** Flips #1/#2/#3 committed (`REALTIME_COMBAT_ENABLED` now live on World-1). **Before flip #4, do the outstanding browser smoke for flip #3:** `npm run dev` → on a World-1 floor (1–10) fight a field encounter (answer → auto-attack; Counter/Support/Item; Flee) and the floor-10 boss (verify the 3 readable phase channels — arena/boss color + phase banner + attack pattern — on desktop, mobile, and reduced-motion); confirm floors 11+ still open the turn-based modal; confirm rewards/HP/MP persist correctly across a reload. **Next flip = #4 `KNOWLEDGE_BREAK_ENABLED` (`app/data/learning/knowledgeBreak.ts`)** — requires wiring the `KnowledgeBreakController` into `RealtimeBattle`'s loop (pause monster assault while a break is open; feed answers to mastery via `applyAnswer`/`summarizeSession`; emit a learning summary at fight end). Then `CLASS_KITS` → `SIGILS` → `ADAPTIVE_EXPEDITIONS`. Gate (full phase): new player finishes World 1 boss; no soft lock; mobile flow; learning summary produced; readable boss phases; no placeholder-critical areas. See `docs/execution/PHASE_14_PLAN.md` §3–§4.
