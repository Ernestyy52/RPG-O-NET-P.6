...
- **Phase 13 — Progression / loot / crafting / Sigils** (next; depends 04+07). See `docs/foundation/MIGRATION_SEQUENCE.md`.
- Phases 00–12 complete and committed. P07 combat domain `app/data/combat/` (now includes `classKits.ts`). P08 zone runtime `app/game/runtime/`. P09 `RealtimeCombat`. P10 `KnowledgeBreakController`. P11 `generateExpedition`/`evaluateExpedition`. P12 class kits (`app/data/combat/classKits.ts`) — 4 distinct active kits, save v2→v3 adds `kitLoadout`. Flags `COMBAT_DOMAIN_ENABLED` + `NEW_ZONE_RUNTIME_ENABLED` + `REALTIME_COMBAT_ENABLED` + `KNOWLEDGE_BREAK_ENABLED` + `ADAPTIVE_EXPEDITIONS_ENABLED` + `CLASS_KITS_ENABLED` all off/dormant. Save version now 3 (envelope still dormant, legacy `player` authoritative). Nothing wired into a rendered scene/UI/live store yet (integrates at World 1, Phase 14). NOTE: P13 also touches persisted shape (Sigils/crafting inventory) → likely a v3→v4 migration; idempotent claims required (gate: no duplicate claims, no negative currency).
