...
// ขนาดเฟรมย้ายไปโมดูล pure (heroFrames.ts) ให้ scale lab/test ใช้ร่วมโดยไม่ลาก Phaser
import { CLASS_SHEETS } from './heroFrames'
export { CLASS_SHEETS, HERO_SRC_H } from './heroFrames'
 ...
 *  ค่าจริงอยู่ที่ scale contract (S4) — ที่นี่ re-export ให้ผู้บริโภคเดิมไม่ต้องแก้ import */
import { HERO_VISUAL_H, PLAYER_BODY_WIDTH, PLAYER_BODY_HEIGHT, PLAYER_FEET_OFFSET } from '../scaleContract'
export const HERO_DISPLAY_H = HERO_VISUAL_H

/**
 * Hitbox มาตรฐานเท่ากันทุกคลาส (~18x12 world px ที่เท้า) — เฟรมแต่ละคลาสกว้างไม่เท่ากัน
 * ถ้าผูก hitbox กับขนาดเฟรมจะไม่แฟร์และ sync กับ server ยาก จึง fix เป็นค่าโลกเดียว (scale contract)
 */
export function applyStandardHeroBody(sprite: Phaser.Physics.Arcade.Sprite, scale: number) {
  const body = sprite.body as Phaser.Physics.Arcade.Body
  const w = PLAYER_BODY_WIDTH / scale
  const h = PLAYER_BODY_HEIGHT / scale
  body.setSize(w, h)
  body.setOffset((sprite.width - w) / 2, sprite.height - h - PLAYER_FEET_OFFSET / scale)
}
...
// base-path utility moved to assetBase.ts (no Phaser dependency) — re-export keeps the old API
export { setAssetBase, assetPath } from './assetBase'
...
export function buildHeroAnimations(scene: Phaser.Scene) {
