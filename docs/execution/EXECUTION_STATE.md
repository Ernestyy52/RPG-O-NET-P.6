...
- **Phase status:** Inc 1 (plan) + Inc 2 (DungeonScene) done; **Inc 3 in progress** — pure boss 3-phase state machine done (+tests); **two live flag flips landed: `NEW_ZONE_RUNTIME_ENABLED=true` (flip #1), `COMBAT_DOMAIN_ENABLED=true` (flip #2)** — BattleModal now drives turn math through the engine resolvers (`resolveHeroSkill`/`resolveMonsterAttack`) on all floors; parity tests assert engine output == legacy formulas. Remaining Inc 3: `REALTIME_COMBAT_ENABLED` → `KNOWLEDGE_BREAK_ENABLED` → `CLASS_KITS_ENABLED` → `SIGILS_ENABLED` → `ADAPTIVE_EXPEDITIONS_ENABLED` (one flag/commit, each needs a real in-browser battle smoke) + BossScene telegraph rendering. Inc 4–5 pending. Build exit 0, 196 tests. **Human-at-browser step outstanding: turn-based battle smoke to confirm flip #2 feel/behavior before flip #3.** See `docs/execution/PHASE_14_PLAN.md` + `DECISION_LOG.md` D-011.
...
**Continue Phase 14 Inc 3 flag flips.** Flips #1 (`NEW_ZONE_RUNTIME_ENABLED`) + #2 (`COMBAT_DOMAIN_ENABLED`) committed. **Outstanding human step before flip #3:** run `npm run dev`, fight a normal floor battle (attack/counter/support/potion, win + lose) to confirm engine-driven turns feel/behave identically (per D-010). Next flip = `REALTIME_COMBAT_ENABLED` (`app/data/combat/index.ts:21`) — but it first needs the remaining Inc 3 engineering: realtime action-lite loop wired into DungeonScene + BossScene 3-phase telegraph rendering (state machine `app/data/combat/bossPhases.ts` already built/tested). After it: `KNOWLEDGE_BREAK` → `CLASS_KITS` → `SIGILS` → `ADAPTIVE_EXPEDITIONS`. Gate (full phase): new player finishes World 1 boss; no soft lock; mobile flow; learning summary produced; readable boss phases; no placeholder-critical areas. See `docs/execution/PHASE_14_PLAN.md` §3.
