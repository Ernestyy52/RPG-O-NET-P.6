...
**Continue Phase 14 Inc 3 → flip #5.** Flips #1–#4 committed. **Accumulating browser-smoke debt (do before flip #5):** `npm run dev` → World-1 floor (1–10): (a) field fight — answer→auto-attack, Counter/Support/Item, Flee; (b) floor-10 boss — 3 readable phase channels (color+banner+pattern) on desktop/mobile/reduced-motion; (c) Knowledge Break interrupts a fight, the monster pauses, empower/combo-lost applies, and a learning summary shows + persists across reload; (d) floors 11+ still open the turn-based modal; (e) rewards/HP/MP persist across reload. **Next flip = #5 `CLASS_KITS_ENABLED` (`app/data/combat/classKits.ts`)** — four class kits drive skills/resources in engine combat (both BattleModal-engine and realtime). Then `SIGILS` → `ADAPTIVE_EXPEDITIONS`. Gate (full phase): new player finishes World 1 boss; no soft lock; mobile flow; learning summary produced; readable boss phases; no placeholder-critical areas. See `docs/execution/PHASE_14_PLAN.md` §3–§4.
